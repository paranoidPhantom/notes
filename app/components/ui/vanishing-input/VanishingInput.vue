<template>
    <form
        :class="[
            'relative mx-auto h-12 w-full max-w-xl overflow-hidden rounded-full bg-card shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),_0px_1px_0px_0px_rgba(25,28,33,0.02),_0px_0px_0px_1px_rgba(25,28,33,0.08)] transition duration-200 ring-accent ring',
            vanishingText && 'bg-secondary',
        ]"
    ...
    >
        ...
        <!-- Placeholder Text -->
        <div
            class="flex pointer-events-none absolute inset-0 items-center rounded-full"
        >
            <Transition
            ...
            >
                ...
                >
                    ...
                    <span class="inline lg:hidden">Коллоквиум проходной</span>
                </p>
            </Transition>
        </div>
    </form>
...
const props = withDefaults(defineProps<Props>(), {
    placeholders: () => [
        "Какой проходной на коллоквиум?",
        "Что такое факторпространство и с чем его едят?",
        "Гомоморфизмы запрещены на территории РФ?",
        "Что делать, если определитель равен нулю?",
        "Как объяснить маме, что такое ортонормированность?",
        "Почему линейная оболочка не продаётся в магазине?",
        "Евклидово пространство — это район в Греции?",
        "Как доказать, что я линейно зависим от кофе?",
    ],
});
